...
const LOG_DIRNAME = path_1.default.resolve(os_1.default.tmpdir(), `applitools-logs`);
async function makeServer({ debug = false, idleTimeout = IDLE_TIMEOUT, ...serverConfig } = {}) {
    var _a;
    const { server, port } = await (0, handler_1.makeHandler)(serverConfig);
    ...
    server.on('connection', client => {
        ...
        const socket = (0, tracker_1.withTracker)({
            debug,
            socket: (0, socket_1.makeSocket)(client),
        });
        clearTimeout(idle);
        socket.on('close', () => {
